...
class Pokemon:
    ...
    async def info(self):
        ...
        if not self.ability:
            self.ability = await self.get_ability()  # Henüz yüklenmemişse yeteneğin geri alınması
        return f"Pokémonunuzun ismi: {self.name} Pokemonunuzun yeteneği: {self.ability}"
