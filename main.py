...
async def info(ctx):
    if ctx.author.name in Pokemon.pokemons:
        ...
        await ctx.send("Pokemonunuz yok. '.go' yazarak pokemon oluşturun.")
# '.sil' komutu
@bot.command()
async def sil(ctx):
    ...
    if author in Pokemon.pokemons.keys():
        ...
        await ctx.send(f"**{author}**, Pokemon'unuz başarıyla silindi!")
    else:
        await ctx.send(f"**{author}**, Henüz oluşturulmuş bir Pokemon'unuz yok!")

# '.reroll' komutu
@bot.command()
async def reroll(ctx):
    author = ctx.author.name
    # Kullanıcı kaydı varsa önce silinir
    if author in Pokemon.pokemons.keys():
        del Pokemon.pokemons[author]
    
    # Yeni bir Pokemon oluşturulur
    pokemon = Pokemon(author)
    await ctx.send(f"**{author}**, Pokemon'unuz yeniden çekildi!\n" + await pokemon.info())
    ...
    if image_url:
        ...
        await ctx.send(f"**{author}**, Pokemon'un görüntüsü yüklenemedi!")
@bot.command()
async def feed(ctx):
    author = ctx.author.name
